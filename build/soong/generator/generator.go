...
func init() {
	android.RegisterModuleType("weeb_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = weebExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = weebExpandVariables(ctx, dep_file)
	...
	exCmd := weebExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "weeb.sbox.textproto"
	desc := "generate"
	name := "generator"
