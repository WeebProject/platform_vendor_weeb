...
ifeq ($(TARGET_BUILD_VARIANT),OFFICIAL)
    ...
    PROD_VERSION += WeebProjekt-$(PLATFORM_WEEB_RELEASE)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-OFFICIAL
else
    PLATFORM_WEEB_VERSION := $(PLATFORM_WEEB_RELEASE)
    PROD_VERSION += WeebProjekt-$(PLATFORM_WEEB_RELEASE)-$(TARGET_DEVICE)-$(shell date +%m%d%H%M)-UNOFFICIAL
endif
